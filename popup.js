...
let bookmarkSearchDebounce = null;
let bookmarkIndex = null;
let bookmarkData = [];
let bookmarkFolderMap = {};
...
function formatTime(seconds) {
  ...
  if (h > 0) {
    parts.push(
      `<span class="time-hours">${h}</span><span class="time-unit">h</span>`,
    );
  }

  if (h > 0 || m > 0) {
    parts.push(
      `<span class="time-minutes">${m}</span><span class="time-unit">m</span>`,
    );
  }

  parts.push(
    `<span class="time-seconds">${s}</span><span class="time-unit">s</span>`,
  );
...
function formatTimeAligned(seconds) {
  ...
  if (h > 0) {
    ...
    parts.push(
      `<span class="time-hours">${hourStr}</span><span class="time-unit">h</span>`,
    );
  } else {
  ...
  if (h > 0 || m > 0) {
    ...
    parts.push(
      `<span class="time-minutes">${minStr}</span><span class="time-unit">m</span>`,
    );
  } else {
    parts.push(`<span>\u2007\u2007</span>`);
  }
  ...
  parts.push(
    `<span class="time-seconds">${secStr}</span><span class="time-unit">s</span>`,
  );
...
function calculateAverages(timeData, period) {
  ...
  dates.forEach((date) => {
    ...
    if (dayData.media) {
      dayMediaSeconds = Object.values(dayData.media).reduce(
        (sum, val) => sum + val,
        0,
      );
    }
    ...
    if (dayData.browsing) {
      dayBrowsingSeconds = Object.values(dayData.browsing).reduce(
        (sum, val) => sum + val,
        0,
      );
    }
...
function displayList(container, totals, type) {
  ...
  if (entries.length === 0) {
    container.innerHTML =
      '<div class="empty-message">No data for this period</div>';
    return;
  }
  ...
  entries.forEach(([host, seconds], index) => {
    ...
    const barWidth =
      maxDuration > 0 ? (seconds / maxDuration) * maxBarWidth : 0;
...
async function deleteAllData() {
  if (
    !confirm(
      "Delete ALL data for all time periods and all settings?\n\nThis cannot be undone!",
    )
  ) {
    return;
  }
...
async function loadPeriod() {
  const { selectedPeriod = "today" } =
    await browser.storage.local.get("selectedPeriod");
...
async function loadWhitelist() {
  ...
  if (whitelist.length === 0) {
    whitelistList.innerHTML =
      '<div class="empty-message">No whitelisted sites</div>';
    return;
  }
...
async function updateBlockToggle() {
  ...
  const { blockSettings = {} } =
    await browser.storage.local.get("blockSettings");
...
async function updateDarkToggle() {
  ...
  const { darkModeSettings = {} } =
    await browser.storage.local.get("darkModeSettings");
...
async function updateRightClickToggle() {
  ...
  const { rightClickAllowed = [] } =
    await browser.storage.local.get("rightClickAllowed");
...
async function loadBlockSettings() {
  ...
  const settings = blockSettings[site] ||
    blockSettings.global || {
      blockImages: false,
      blockSVG: false,
    };
...
async function updateHoverZoomToggleUI() {
  const { hoverZoomEnabled = false } =
    await browser.storage.local.get("hoverZoomEnabled");
...
async function updateClearButtonStates() {
  ...
  if (clearInvertParentBtn) {
    if (
      invertParentSettings[host] &&
      Object.keys(invertParentSettings[host]).length > 0
    ) {
...
async function loadHighlightPalette() {
  const { highlightPaletteIndex = 0 } = await browser.storage.local.get(
    "highlightPaletteIndex",
  );
...
async function loadHighlightTerms() {
  const { highlightTerms = [] } =
    await browser.storage.local.get("highlightTerms");
  displayHighlightTerms(highlightTerms);
}

async function loadHighlightEnabled() {
  const { highlightEnabled: enabled = false } =
    await browser.storage.local.get("highlightEnabled");
...
async function displayHighlightTerms(terms) {
  ...
  if (terms.length === 0) {
    container.innerHTML =
      '<span style="color: #858585; font-size: 0.85rem;">No search terms</span>';
    return;
  }
  ...
  terms.forEach((term, index) => {
    ...
    wrapper.style.cssText =
      "display: inline-flex; align-items: center; gap: 0; margin-right: 0.1rem; margin-bottom: 0.25rem;";
    ...
    removeBtn.style.cssText =
      "min-width: auto; padding: 0; margin: 0; margin-left: 0rem; border: none; background: none; color: #ff3f3f; font-size: 16px; cursor: pointer; line-height: 1;";
...
async function addHighlightTerms(termsText) {
  ...
  const individualTerms = termsWithoutQuotes
    ? termsWithoutQuotes.split(/\s+/).filter((t) => t.length >= 1)
    : [];
  ...
  for (const term of allTerms) {
    ...
    if (
      allTerms.includes(quotedVersion) &&
      allTerms.includes(unquotedVersion)
    ) {
  ...
  if (conflicts.length > 0) {
    alert(
      `Cannot add both exact and partial matches for the same term(s): ${conflicts.join(", ")}\n\nPlease use either "${conflicts[0]}" (exact) OR ${conflicts[0]} (partial), but not both.`,
    );
    return;
  }
  ...
  const allTermsToAdd = [
    ...highlightTerms,
    ...newTerms.filter((t) => !highlightTerms.includes(t)),
  ];
...
async function applyHighlights(terms, paletteIndex) {
  const { diacriticsEnabled = true } =
    await browser.storage.local.get("diacriticsEnabled");
...
async function loadDiacriticsEnabled() {
  const { diacriticsEnabled: enabled = true } =
    await browser.storage.local.get("diacriticsEnabled");
...
async function updateSiteBlockButton() {
  ...
  const { siteBlockTimestamps = {} } = await browser.storage.local.get(
    "siteBlockTimestamps",
  );
...
async function loadBlurSettingsForCurrentSite() {
  ...
  const siteSettings = blurSettings[host] || {
    imageBlur: 8,
    videoBlur: 8,
    invertImages: false,
  };
...
imageBlurSlider.addEventListener("input", async () => {
  ...
  imageBlurDebounce = setTimeout(async () => {
    ...
    const { blurSettings = {} } =
      await browser.storage.local.get("blurSettings");
...
document
  .getElementById("toggleSiteInBlockList")
...
siteBlockToggle.addEventListener("click", async () => {
  ...
  if (blockSettings[selectedSite]) {
    ...
    const stillHasSettings =
      darkModeSettings[selectedSite] ||
      rightClickAllowed.includes(selectedSite);
...
siteDarkToggle.addEventListener("click", async () => {
  ...
  if (darkModeSettings[selectedSite]) {
    ...
    const stillHasSettings =
      blockSettings[selectedSite] || rightClickAllowed.includes(selectedSite);
...
siteDarkEnable.addEventListener("click", async () => {
  ...
  darkModeSettings[selectedSite].enabled =
    !darkModeSettings[selectedSite].enabled;
  await browser.storage.local.set({ darkModeSettings });
...
function toggleSettingsCollapse() {
  ...
  const collapsibleHeader = document.getElementById(
    "collapsibleHighlightHeader",
  );
...
rightClickToggle.addEventListener("click", async () => {
  ...
  if (rightClickAllowed.includes(selectedSite)) {
    ...
    const stillHasSettings =
      blockSettings[selectedSite] || darkModeSettings[selectedSite];
...
blockSiteSelect.addEventListener("change", async () => {
  ...
  if (blockSettings[selectedSite]) {
    ...
    siteBlockToggle.classList.remove("active");
    document.getElementById("siteBlockToggleIcon").textContent = "🧪";
    siteBlockToggle.style.display =
      selectedSite === "global" || selectedSite === "local-files"
        ? "none"
        : "inline-block";
  }
  ...
  if (rightClickAllowed.includes(selectedSite)) {
    ...
    rightClickToggle.classList.remove("active");
    document.getElementById("rightClickToggleIcon").textContent = "🖱️";
    rightClickToggle.style.display =
      selectedSite === "global" || selectedSite === "local-files"
        ? "none"
        : "inline-block";
  }
...
globalToggleBtn.addEventListener("click", async () => {
  const { globalBlockingEnabled = false } = await browser.storage.local.get(
    "globalBlockingEnabled",
  );
...
darkModeToggle.addEventListener("click", async () => {
  const { darkModeEnabled = false } =
    await browser.storage.local.get("darkModeEnabled");
...
maxTabsToggle.addEventListener("click", async () => {
  const { maxTabsEnabled = false } =
    await browser.storage.local.get("maxTabsEnabled");
...
document
  .getElementById("manualBlockInput")
...
document.querySelectorAll(".nav-link").forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    if (settingsCollapsed) {
      ...
      const collapsibleSettings = document.getElementById(
        "collapsibleSettings",
      );
      const formGroup = collapsibleSettings?.previousElementSibling;
...
async function loadSincePalette() {
  const { sincePaletteIndex = 0 } =
    await browser.storage.local.get("sincePaletteIndex");
...
function displaySinceItems(items) {
  ...
  if (items.length === 0) {
    container.innerHTML =
      '<span style="color: #858585; font-size: 0.85rem;">No activities tracked</span>';
    return;
  }
...
document
  .getElementById("clearAllSinceBtn")
  ?.addEventListener("click", clearAllSinceItems);

document
  .getElementById("sinceColorsBtn")
  ?.addEventListener("click", async () => {
    currentSincePaletteIndex =
      (currentSincePaletteIndex + 1) % monochromeListPalettes.length;
  ...
  try {
    ...
    for (const [key, value] of Object.entries(importedData)) {
      if (key === "lists" && value && typeof value === "object") {
        const existingLists = existingData.lists || {
          category1: {},
          category2: {},
        };
        ...
        for (const categoryName in importedLists.category1) {
          if (
            ...
            Array.isArray(existingLists.category1[categoryName])
          ) {
        ...
        for (const categoryName in importedLists.category2) {
          if (
            ...
            Array.isArray(existingLists.category2[categoryName])
          ) {
        ...
        const existingNames = new Set(
          existingSinceItems.map((item) => item.name),
        );

        const newItems = importedSinceItems.filter(
...
document
  ...
  ?.addEventListener("click", async () => {
    ...
    const { invertSettings = {} } =
      await browser.storage.local.get("invertSettings");
...
document
  .getElementById("invertParentBtn")
  ?.addEventListener("click", async () => {
    const tabs = await browser.tabs.query({
      active: true,
      currentWindow: true,
...
document
  .getElementById("clearInvertParentBtn")
  ?.addEventListener("click", async () => {
    const tabs = await browser.tabs.query({
      active: true,
      currentWindow: true,
    ...
    const { invertParentSettings = {} } = await browser.storage.local.get(
      "invertParentSettings",
    );
...
document
  .getElementById("toggleZeroBrowsing")
  ?.addEventListener("click", async () => {
...
document
  .getElementById("toggleZeroMedia")
  ?.addEventListener("click", async () => {
...
document
  .getElementById("highlightTermsInput")
  ?.addEventListener("keypress", async (e) => {
    if (e.key !== "Enter") return;
...
document
  .getElementById("freqHighlightBtn")
  .addEventListener("click", async () => {
...
document
  .getElementById("addHighlightBtn")
  ?.addEventListener("click", async () => {
    const input = document.getElementById("highlightTermsInput");
    await addHighlightTerms(input.value.trim());
    input.value = "";
...
document
  .getElementById("highlightColorsBtn")
  ?.addEventListener("click", async () => {
    currentHighlightPaletteIndex =
      (currentHighlightPaletteIndex + 1) % colorPalettes.length;
...
document
  .getElementById("diacriticsToggle")
  ?.addEventListener("click", async () => {
    diacriticsEnabled = !diacriticsEnabled;
...
async function loadYouTubeSettings() {
  const { youtubeSettings = {} } =
    await browser.storage.local.get("youtubeSettings");
...
async function loadListPalette() {
  const { listPaletteIndex = 0 } =
    await browser.storage.local.get("listPaletteIndex");
...
async function loadLists() {
  const { lists = { category1: {}, category2: {} } } =
    await browser.storage.local.get("lists");
...
function displayListCategories(lists) {
  ...
  if (sortedCat1.length === 0) {
    container1.innerHTML =
      '<span style="color: #858585; font-size: 0.85rem;">No categories</span>';
  }
...
function createCategoryTag(container, categoryName, categoryKey, color, lists) {
  ...
  if (categoryKey === "category1") {
    ...
    removeBtn.className = "whitelist-remove";
    removeBtn.style.cssText =
      "min-width: auto; padding: 0.25rem 0.25rem; color: #ff3f3f;";
    removeBtn.textContent = "×";
    removeBtn.title = "Remove term";
...
function displayListItems(items) {
  ...
  if (items.length === 0) {
    container.innerHTML =
      '<span style="color: #858585; font-size: 0.85rem;">No items</span>';
    return;
  }
...
async function removeListItem(itemName) {
  ...
  if (lists[currentListKey][currentListCategory]) {
    lists[currentListKey][currentListCategory] = lists[currentListKey][
      currentListCategory
...
async function loadListCategoryPalette() {
  const { listCategoryPaletteIndex = 0 } = await browser.storage.local.get(
    "listCategoryPaletteIndex",
  );
...
document.getElementById("toggleCJK").addEventListener("click", async () => {
  ...
  if (cjkEnabled && !diacriticsEnabled) {
    ...
    const diacriticsToggleText = document.getElementById(
      "diacriticsToggleText",
    );
...
function updateCJKButton() {
  ...
  if (cjkEnabled) {
    ...
    btn.style.color = "";
    btn.title =
      "CJK Mode: OFF (Chinese/Japanese/Korean/Arabic) or show partial term only)";
    btnText.textContent = "\u4E2D\u65E5\u97D3"; // 中日韓
  }
}
...
document.getElementById("bookmarkSearchBtn")?.addEventListener("click", () => {
...
document
  .getElementById("closeBookmarkSection")
...
async function buildBookmarkIndex() {
  ...
  function walkTree(nodes, folderPath) {
    for (const node of nodes) {
      ...
      if (node.children) {
        const currentFolder = node.title || "Root";
        const newPath = folderPath
          ? `${folderPath} > ${currentFolder}`
          : currentFolder;
        walkTree(node.children, newPath);
      }
    }
  }
  ...
  bookmarkIndex = new MiniSearch({
    fields: ["title", "url", "host", "folder"],
    storeFields: ["title", "url", "host", "folder"],
    searchOptions: {
      boost: { title: 3, host: 2, folder: 1.5, url: 1 },
      fuzzy: 0.2,
      prefix: true,
    },
  });

  bookmarkIndex.addAll(bookmarkData);

  const countEl = document.getElementById("bookmarkResults");
  countEl.innerHTML = `<div class="bookmark-result-count">${bookmarkData.length} bookmarks indexed</div>`;
}

function searchBookmarks(query) {
  if (!bookmarkIndex || !query.trim()) {
    document.getElementById("bookmarkResults").innerHTML = "";
    return;
  }

  const mode = document.getElementById("bookmarkSearchMode").value;
  ...
  const results = bookmarkIndex.search(query, searchOpts);
  displayBookmarkResults(results, query);
}

function displayBookmarkResults(results, query) {
  const container = document.getElementById("bookmarkResults");
  container.innerHTML = "";

  if (results.length === 0) {
    container.innerHTML =
      '<div class="bookmark-no-results">No bookmarks found</div>';
    return;
  }

  const countDiv = document.createElement("div");
  countDiv.className = "bookmark-result-count";
  ...
  results.slice(0, 100).forEach((result) => {
    ...
    item.className = "bookmark-result-item";
    item.href = result.url;
    item.target = "_blank";
    item.rel = "noopener";

    const normalizedScore = Math.round((result.score / maxScore) * 100);

    const folderName = result.folder ? result.folder.split(" > ").pop() : "";

    const title = highlightBookmarkMatch(
      escapeHtml(result.title || "Untitled"),
      query,
    );
    const url = highlightBookmarkMatch(escapeHtml(result.url), query);

    item.innerHTML = `
            <div class="bookmark-result-top">
                <span class="bookmark-result-name">${title}</span>
                ${folderName ? `<span class="bookmark-result-folder" data-folder="${escapeHtml(result.folder)}">${escapeHtml(folderName)}</span>` : ""}
                <span class="bookmark-result-score">${normalizedScore}</span>
            </div>
            <div class="bookmark-result-bottom">
                <span class="bookmark-result-url">${url}</span>
                <span class="bookmark-result-open">↗</span>
            </div>
        `;
    ...
    const folderEl = item.querySelector(".bookmark-result-folder");
    if (folderEl) {
      folderEl.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        showFolderBookmarks(result.folder);
      });
    }
...
function showFolderBookmarks(folderPath) {
  const container = document.getElementById("bookmarkResults");
  container.innerHTML = "";

  const folderBookmarks = bookmarkData.filter((bm) => bm.folder === folderPath);
  const folderName = folderPath ? folderPath.split(" > ").pop() : "Root";

  const header = document.createElement("div");
  header.style.cssText =
    "display: flex; align-items: center; gap: 8px; margin-bottom: 8px; padding-bottom: 6px; border-bottom: 1px solid #333;";
  header.innerHTML = `
        <button id="bookmarkFolderBack" class="blocking-icon-btn" style="font-size: 0.8rem; min-width: 24px; width: 24px; height: 24px;">←</button>
        <span style="color: #6c91c2; font-size: 12px; font-weight: 600;">${escapeHtml(folderName)}</span>
        <span class="bookmark-result-count" style="margin: 0; padding: 0;">${folderBookmarks.length} bookmark${folderBookmarks.length !== 1 ? "s" : ""}</span>
    `;
  container.appendChild(header);

  document
    .getElementById("bookmarkFolderBack")
    .addEventListener("click", () => {
      const query = document.getElementById("bookmarkSearchInput").value;
      if (query.trim()) {
        searchBookmarks(query);
      } else {
        container.innerHTML = `<div class="bookmark-result-count">${bookmarkData.length} bookmarks indexed</div>`;
      }
    });

  if (folderBookmarks.length === 0) {
    container.innerHTML +=
      '<div class="bookmark-no-results">No bookmarks in this folder</div>';
    return;
  }
  ...
  folderBookmarks.forEach((bm) => {
    ...
    item.innerHTML = `
            <div class="bookmark-result-top">
                <span class="bookmark-result-name">${escapeHtml(bm.title || "Untitled")}</span>
                <button class="bookmark-delete-btn" data-id="${bm.id}" title="Delete bookmark">🗑</button>
            </div>
            <div class="bookmark-result-bottom">
                <span class="bookmark-result-url">${escapeHtml(bm.url)}</span>
                <span class="bookmark-result-open">↗</span>
            </div>
        `;
    ...
    deleteBtn.addEventListener("click", async (e) => {
      ...
      bookmarkData = bookmarkData.filter((b) => b.id !== bm.id);
      bookmarkIndex.remove(bm);

      item.remove();

      const countEl = header.querySelector(".bookmark-result-count");
      const remaining = container.querySelectorAll(
        ".bookmark-result-item",
      ).length;
      countEl.textContent = `${remaining} bookmark${remaining !== 1 ? "s" : ""}`;
    });
...
function highlightBookmarkMatch(text, query) {
  const terms = query
    .toLowerCase()
    .split(/\s+/)
    .filter((t) => t.length > 0);
  ...
  terms.forEach((term) => {
    ...
    result = result.replace(
      regex,
      '<mark style="background:#4a6fa5;color:#fff;padding:0 1px;border-radius:2px;">$1</mark>',
    );
  });
...
document
  ...
  ?.addEventListener("input", (e) => {
    if (bookmarkSearchDebounce) clearTimeout(bookmarkSearchDebounce);
    bookmarkSearchDebounce = setTimeout(() => {
...
document
  .getElementById("bookmarkSearchMode")
...
document
  .getElementById("listCategoryInput")
  ?.addEventListener("keypress", async (e) => {
    if (e.key !== "Enter") return;
...
document
  .getElementById("addListCategoryBtn")
  ?.addEventListener("click", async () => {
    const input = document.getElementById("listCategoryInput");
    await addListCategory(input.value.trim());
    input.value = "";
...
document
  .getElementById("clearAllListsBtn")
  ?.addEventListener("click", clearAllLists);

document
  .getElementById("listCategoryColorsBtn")
  ?.addEventListener("click", async () => {
    currentListCategoryPaletteIndex =
      (currentListCategoryPaletteIndex + 1) % colorPalettes.length;
...
document
  .getElementById("listItemColorsBtn")
  ?.addEventListener("click", async () => {
    currentListPaletteIndex =
      (currentListPaletteIndex + 1) % monochromeListPalettes.length;
...
function toggleSettingsCollapse() {
  ...
  if (settingsCollapsed) {
    document
      .getElementById("collapsibleSettings")
      ?.classList.add("settings-collapsed");
    applyCollapsedStyles();
  } else {
    document
      .getElementById("collapsibleSettings")
      ?.classList.remove("settings-collapsed");
...
document
  ...
  ?.addEventListener("click", toggleSettingsCollapse);
...
document
  .getElementById("closeListItems")
  ?.addEventListener("click", closeListItemsSection);
